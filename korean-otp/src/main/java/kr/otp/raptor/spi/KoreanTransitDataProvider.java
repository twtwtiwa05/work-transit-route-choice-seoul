...
/**
 * 한국 대중교통용 Raptor Transit Data Provider.
 *
 * ★★★ Raptor SPI의 최상위 인터페이스 ★★★
 *
 ...
 * - 비용 계산기, 슬랙 제공자
 *
 * 이 클래스는 TransitData를 래핑하여 Raptor가 요구하는 형태로 제공.
 */
public class KoreanTransitDataProvider
    ...
    public KoreanTransitDataProvider(TransitData data) {
        this(data, CalibrationConfig.defaults());
    }

    public KoreanTransitDataProvider(TransitData data, CalibrationConfig config) {
        this.data = data;
        this.costCalculator = new KoreanCostCalculator(config);
    ...
    @Override
    public RaptorConstrainedBoardingSearch<KoreanTripSchedule> transferConstraintsForwardSearch(
        int routeIndex
    ) {
        // 제약 환승 미사용 - No-op 반환
    ...
    @Override
    public RaptorConstrainedBoardingSearch<KoreanTripSchedule> transferConstraintsReverseSearch(
        int routeIndex
    ) {
        // 제약 환승 미사용 - No-op 반환
        return NoopConstrainedBoardingSearch.INSTANCE;
    ...
    public String toString() {
        return "KoreanTransitDataProvider{" +
            "stops=" + data.getStopCount() +
            ", routes=" + data.getRouteCount() +
            ", trips=" + data.getTotalTripCount() +
            '}';
    }
}
