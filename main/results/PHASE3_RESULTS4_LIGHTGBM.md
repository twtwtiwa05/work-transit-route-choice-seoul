...
SHAP 중요도 순위는 T_walk > N_transfer > T_ride > D_subway이다.

> **정정 (2026-08-16)**: 이전 버전의 "MNL β 절대값 순위와 완벽히 일치" 주장은 오류.
> 원시 |β| 순위는 N_transfer > D_subway > T_walk > T_ride로 SHAP과 ρ = 0.0이다
> (단위가 달라 비교 자체가 무의미). 변동폭으로 표준화한 |β|·SD 기준으로는
> 상위 2개(보행·환승)가 일치하며 ρ = 0.8 — 상세는 PHASE3_RESULTS5 §5 참조.
...
| 검증 방법 | 근거 | 결론 |
|-----------|------|------|
| SHAP 순위 vs 표준화 MNL \|β\|·SD 순위 | 상위 2개(T_walk, N_transfer) 일치, ρ = 0.8 | 효용함수 변수 선택 적절 |
...
2. **SHAP-β 교차검증**: 트리 기반 비모수적 변수 중요도가 변동폭 표준화(|β|·SD)한 효용함수 계수 중요도와 지배적 2개 요인(보행·환승)에서 일치(ρ = 0.8)하여, 효용함수 사양(specification)의 타당성을 독립적으로 확인.
